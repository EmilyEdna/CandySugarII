...
namespace CandySugar.Library.Platforms.Android.CrossDown
{
    ...
    public class DownReceiver : BroadcastReceiver
    {
        public override void OnReceive(Context context, Intent intent)
        {
            ...
            var downloadFile = AndroidCommon.Downs. CrossDown.Current.Queue.Cast<DownFile>().FirstOrDefault(f => f.Id == reference);
            ...
            try
            {
                using (var cursor = ((DownloadManager)context.GetSystemService(Context.DownloadService)).InvokeQuery(query))
                {
                    ...
                    {
                        ((DownManager)AndroidCommon.Downs.CrossDown.Current).UpdateFileProperties(cursor, downloadFile);
                    }
                    cursor?.Close();
                }
            }
